...
Bu branch, `class_registry.yaml`/`ACTION_ENTER_SHOOTING_ZONE` ile birlikte URDF/bridge değişiklikleri de getiriyor. `d435i_link`→`d435if_link` rename'i onaylanıp uygulandı (aşağıda), ama kalan ikisi **hâlâ bilerek merge edilmedi** çünkü bizim repoyla gerçek çakışmaları var — ekip kararı olmadan uygulanmamalı:
...
2. **Tekerlek geometrisi çelişkisi**: o branch'te visual radius 0.165→0.10 değişmiş ama collision hâlâ 0.165 (kendi içinde bile tutarsız) — bizim repo ise mekanik ekipten gelen 0.200m değerine göre STL'den gömülü tekerleği çıkarmıştı (bkz. "çifte tekerlek" notu). İki taraf artık uyumsuz.
3. **`rear_camera_link`'in Gazebo `<sensor>` bloğu o diff'te silinmiş** ama `gazebo.launch.py`'ye `/rear_camera/image_raw` + `/rear_camera/camera_info` bridge satırları eklenmiş — sensörü olmayan bir topic'e bridge kuruluyor, veri akmaz, hata da vermez (RTAB-Map kurulumunda bulunanlarla aynı sessiz-başarısızlık kalıbı).
