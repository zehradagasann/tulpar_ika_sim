...
**Yeni bulgu (`274f4f1` ile birlikte geldi)**: aynı commit'te `lidar_joint`/`d435if_joint`/`rpi_hq_camera_joint`'in `origin xyz` değerleri de değişmiş (lidar `0.30 0 0.40`→`0.58 -0.015 0.61`, d435if `0.62 0 0.20`→`0.58 0 0.53`, rpi_hq `0.58 0 0.28`→`0.58 -0.03 0.58` — üçü de öne/yukarı toplanmış, muhtemelen gerçek bir sensör direği/braket tasarımını yansıtıyor). Bizim repo hâlâ eski konumlarda. **Henüz karar verilmedi — mekanik kaynaklı mı teyit edilmeli.**
