...
- Sensors (hepsi Zehra'nın gerçek Gazebo sensor plugin'leriyle bağlı): YDLIDAR TG30 on `lidar_link` (`gpu_lidar`, topic `/scan`, 720 samples, 30m), RealSense D435if on `d435i_link` (`depth_camera`, topic `/d435if/depth/image_raw` — 16 Temmuz'da Zehra'nın doğruladığı çalışan isme güncellendi, bkz. Kamera Routing tablosu), Pi HQ Camera+16mm (atış/nişan) on `rpi_hq_camera_link` (`camera`, topic `/rpi_hq_camera/image_raw`), SJCAM SJ4000 (arka izleme, 180° geriye dönük) on `rear_camera_link` (`camera`, topic `/rear_camera/image_raw`)
...
|---|---|---|---|
| RealSense D435if | `d435i_link` | `/d435if/depth/image_raw`, `/d435if/depth/camera_info`, `/d435if/depth/image_raw/points` | **Derinlik / point cloud** — engel algılama, RTAB-Map'in 3D görsel katmanı. RGB yok, sadece depth. |
...
### ⚠️ Zehra'nın `feature/zehra-slalom-core` branch'inden BİLEREK MERGE EDİLMEYENLER (17 Temmuz 2026)
Bu branch, `class_registry.yaml`/`ACTION_ENTER_SHOOTING_ZONE` ile birlikte URDF/bridge değişiklikleri de getiriyor. O ikisi senkronlandı (yukarıda), ama aşağıdaki üçü **bilerek merge edilmedi** çünkü bizim repoyla gerçek çakışmaları var — ekip kararı olmadan uygulanmamalı:
1. **`d435i_link` → `d435if_link` yeniden adlandırması**: bizim reponun her yerinde (URDF, bridge, RTAB-Map, kamera routing tablosu) `d435i_link` kullanılıyor. Görülmeden merge edilirse TF ağacı ve bridge sessizce kırılır.
