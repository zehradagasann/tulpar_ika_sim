from setuptools import find_packages, setup

package_name = 'tulpar_safety'

setup(
    name=package_name,
    version='0.0.1',
    packages=find_packages(exclude=['test']),
    data_files=[
        ('share/ament_index/resource_index/packages',
            ['resource/' + package_name]),
        ('share/' + package_name, ['package.xml']),
        ('share/' + package_name + '/launch', ['launch/collision_checker.launch.py']),
    ],
    install_requires=['setuptools'],
    zip_safe=True,
    maintainer='talha',
    maintainer_email='talha.dag2005@gmail.com',
    description=(
        'Nav2 /cmd_vel cikisini local costmap ile denetleyip /cmd_vel_safe '
        'olarak yayinlayan Collision Checker (KTR 3.3.2).'
    ),
    license='Apache-2.0',
    tests_require=['pytest'],
    entry_points={
        'console_scripts': [
            'collision_checker = tulpar_safety.collision_checker_node:main',
            'yon_duzeltme_node = tulpar_safety.yon_duzeltme_node:main',
            'egim_feedforward_node = tulpar_safety.egim_feedforward_node:main',
            'yan_egim_izleme_node = tulpar_safety.yan_egim_izleme_node:main',
        ],
    },
)
